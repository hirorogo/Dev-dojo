...
2. *.ta ファイルを削除
3. S1モードでnewflasherフラッシュ（ブートローダーもGlobal版になる）
4. xperableをデフォルト設定で実行 → そのまま成功
```

この順番なら、サイズ修正もhitcodeパッチもTA変更も全部不要で、文鎮化も起きなかった。

:::tip BLロック状態でもGlobalファームは焼ける
「BLアンロック前にファーム焼けるの？」と思うかもしれないが、S1モード（緑LED）でのファームウェアフラッシュはBLロック状態でも問題なく動作する。S1フラッシュとfastbootのBLアンロックは全く別の仕組みなので、**BLロックのまま先にGlobalファームを入れて、その後xperableでBLアンロック**という順番が可能。
:::
